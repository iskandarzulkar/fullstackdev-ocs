  ...
  seed: async (request) => {
    await request.query(`
      ...
      IF NOT EXISTS (SELECT 1 FROM WorkflowApproval WHERE Modul = 'Transaction' AND [Type] = 'HRIS')
        INSERT INTO WorkflowApproval (Modul_id, Modul, [Type], [Value], [Level], Nik, Name, Email, Position)
        VALUES (1, 'Transaction', 'HRIS', 1, 1, NULL, NULL, NULL, NULL);
      ...
      IF NOT EXISTS (SELECT 1 FROM WorkflowApproval WHERE Modul = 'Transaction' AND [Type] = 'Total Amount <=' AND Nik = '74011306950008')
        INSERT INTO WorkflowApproval (Modul_id, Modul, [Type], [Value], [Level], Nik, Name, Email, Position)
        VALUES (1, 'Transaction', 'Total Amount <=', 10000000, 1, '74011306950008', 'Agus Pratama', 'agus.pratama@xyz.co.id', 'Staff IT');
    `);
  },
